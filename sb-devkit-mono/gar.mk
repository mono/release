...
FILELIST ?= installed_files
FILELIST_TARGET ?= installed_files_target

install: build xtermset-install $(addprefix dep-$(GARDIR)/,$(INSTALLDEPS)) $(FILELIST)-wrapper $(FILELIST_TARGET)-wrapper  pre-install $(INSTALL_TARGETS) post-install $(DO_BUILD_CLEAN) $(if $(filter $(USE_STOW),yes),stow)
	@if ! test -e $(WORKDIR)/$(FILELIST); then \
		echo "Scanning file system for new files..."; \
		find $(prefix) -cnewer $(COOKIEDIR)/$(FILELIST) -type f -printf "%p\n" -o \
						-cnewer $(COOKIEDIR)/$(FILELIST) -type l -printf "%p\n" |  \
		sed 's/$(subst /,\/,$(prefix))\///' > $(WORKDIR)/$(FILELIST); \
		echo "Added `awk 'END{print NR}' $(WORKDIR)/$(FILELIST)` new files to \"$(WORKDIR)/$(FILELIST)\"."; \
	fi;
	@if ! test -e $(WORKDIR)/$(FILELIST_TARGET); then \
		echo "Scanning target for new files..."; \
		target_dir=`cat /targets/links/scratchbox.config | grep SBOX_TARGET_DIR= | cut -d "=" -f 2`; \
		find $$target_dir -cnewer $(COOKIEDIR)/$(FILELIST_TARGET) -type f -printf "%p\n" -o \
						-cnewer $(COOKIEDIR)/$(FILELIST_TARGET) -type l -printf "%p\n" |  \
		sed "s£^$$target_dir££" > $(WORKDIR)/$(FILELIST_TARGET); \
		echo "Added `awk 'END{print NR}' $(WORKDIR)/$(FILELIST_TARGET)` new files to \"$(WORKDIR)/$(FILELIST_TARGET)\"."; \
	fi;
	@if [ "x" != "x$(DEBIAN_PACKAGES)" ] ; then \
		echo "Writing package record..."; \
		if [ ! -d $(prefix)/deb_list ] ; then \
			mkdir -p $(prefix)/deb_list; \
		fi; \
		if [ x"$(DEBIAN_PACKAGES)" != x ]; then \
			src=$(DEBIAN_SOURCE); \
			if [ x"$$src" = x ]; then \
				src=$(GARNAME); \
			fi; \
			...
			echo "Source: $$src" > $(prefix)/deb_list/$(GARNAME); \
			echo "Version: $$ver" >> $(prefix)/deb_list/$(GARNAME); \
			echo -n "Binary:" >> $(prefix)/deb_list/$(GARNAME); \
			for pkg in $(DEBIAN_PACKAGES) ; do \
				echo -n " $$pkg" >> $(prefix)/deb_list/$(GARNAME); \
			done; \
			echo >> $(prefix)/deb_list/$(GARNAME); \
		fi; \
	fi
	@if [ "x" != "x$(CREATE_BIN_PACKAGE)" ] ; then \
		for i in `cat $(WORKSRC)/../installed_files`; do echo "$(prefix)/$$i" >> $(WORKSRC)/../installed_files_full_path; done; \
		tar -czf $(INSTALLPKGS)/$(GARNAME)-$(GARVERSION)-bin.tar.gz -T $(WORKSRC)/../installed_files_full_path; \
	fi;
	$(DONADA)

#
# Make "pre-install cookie"
#
$(FILELIST)-wrapper: $(FILELIST)
	@ if ! test -e $(WORKDIR)/$(FILELIST); then \
		sleep 1; \
		echo "Ready to proceed with filelist..."; \
	fi

$(FILELIST):
	@ if ! test -e $(COOKIEDIR)/$(FILELIST); then \
		echo "Making pre-install cookie for filelist..."; \
		$(MAKECOOKIE); \
	fi

$(FILELIST_TARGET)-wrapper: $(FILELIST_TARGET)
	@ if ! test -e $(WORKDIR)/$(FILELIST_TARGET); then \
		sleep 1; \
		echo "Ready to proceed with target filelist..."; \
	fi

$(FILELIST_TARGET):
	@ if ! test -e $(COOKIEDIR)/$(FILELIST_TARGET); then \
		echo "Making pre-install cookie for target filelist..."; \
		$(MAKECOOKIE); \
	fi
