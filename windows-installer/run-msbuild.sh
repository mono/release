...
function msbuild
{
   report "Running MSBuild"
   pushd $CHECKOUT_ROOT
   MSBUILD=$(cygpath -au "C:\Windows\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe")
   SOLUTION="msvc/2012/Mono.sln"

   $MSBUILD $SOLUTION || error "Cannot run MSBuild"
   popd
}
