...
Summary:        A .NET Runtime Environment optimized for Moonlight 2.0
...
%configure \
  ...
  --without-ikvm-native \
  --without-mcs-docs \
  --enable-minimum=com,attach,large_code
make
...
# Remove unused files
rm -rf $RPM_BUILD_ROOT%_prefix/lib/mono/4.0
rm $RPM_BUILD_ROOT%_libdir/libMonoPosixHelper.a
rm $RPM_BUILD_ROOT%_libdir/libMonoPosixHelper.la
rm -rf $RPM_BUILD_ROOT%_prefix/lib/mono/gac/Mono.Security.Win32/[12]*
...
rm $RPM_BUILD_ROOT%_libdir/mono/compat-4.0/ICSharpCode.SharpZipLib.dll
rm $RPM_BUILD_ROOT%_libdir/mono/gac/Mono.Security.Win32/4.0.0.0__0738eb9f132ed756/Mono.Security.Win32.dll*
